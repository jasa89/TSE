...
<div class="content-posts" id="content"> <!--Sisältöalue Posts sivulla oma muotoilu asettelu-->
 ...
 <div class="row"> <!-- Rivi jossa columnit on jaettu osiin-->
  ...
  <?php
    ...
    $sql = "SELECT * FROM posts WHERE date BETWEEN '2023-04-01 ' AND '2023-04-30 ' ORDER BY date DESC";
    ...
    if ($result->num_rows > 0) {
      ...
      while($row = $result->fetch_assoc()) {
        ...
        echo "<div class='card' id='postcard'><h2>"; 
      echo "<br><br>";  
      echo '<a id="a-heading" href="/tse/read.php?id='. $row['id']. '">'. $row['heading']. '</a>'; /* Otsikko jossa linkki otsikon osoittamaan postaukseen */
        echo "</h2>";   
        echo "<h5>";
        echo ($row['date']."<br><br>");/* päivämäärä jolloin postaus on päivitetty*/
            echo "</h5>";  
             echo nl2br($row['abstract']."<br><br>");/* Postien rivivälit rivivälit*/
             echo ' <a href="/tse/read.php?id='. $row['id'] .'" class="button" title="View Post" data-toggle="tooltip">Read more...</a>';
             echo "</div>";
          echo "</div>";  
